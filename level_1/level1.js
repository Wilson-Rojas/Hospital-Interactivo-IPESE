...
function mostrarModal(mensaje) {
  ...
  modal.show();
    ...
    function efectoApagadoRetro(callback) {
    const overlay = document.getElementById("apagado-overlay");

   overlay.classList.add("active");

  // Línea blanca animada
  const line = document.createElement("div");
  line.classList.add("tv-shutoff-line");
  document.body.appendChild(line);

  // Quitar línea y ejecutar callback después
  setTimeout(() => {
    line.remove();
    if (typeof callback === "function") callback();
  }, 600); // Debe coincidir con la duración de shutoffLine
}
